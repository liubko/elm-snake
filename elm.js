...
Elm.SnakeGame = Elm.SnakeGame || {};
Elm.SnakeGame.make = function (_elm) {
   "use strict";
   _elm.SnakeGame = _elm.SnakeGame || {};
   if (_elm.SnakeGame.values)
   return _elm.SnakeGame.values;
   var _op = {},
   _N = Elm.Native,
   _U = _N.Utils.make(_elm),
   _L = _N.List.make(_elm),
   ...
   80,
   79);
   var fieldColor = A3($Color.rgb,
   234,
   247,
   196);
   var txt = F2(function (f,
   string) {
      return $Graphics$Element.leftAligned(f($Text.monospace($Text.color(textColor)($Text.fromString(string)))));
   });
   var incrementSnake = function (_v0) {
      return function () {
         return function () {
            var tail = $List.take(1)($List.reverse(_v0.body));
            var newBody = A2($List.append,
            _v0.body,
            tail);
   ...
   var getSnakeHead = function (snake) {
      return A2($Maybe.withDefault,
      {_: {},x: 0,y: 0},
      $List.head(snake.body));
   };
   var checkForSnakeCollision = function (snake) {
      return function () {
         var head = getSnakeHead(snake);
         return $List.any(function (a) {
            return _U.eq(a.x,
            head.x) && _U.eq(a.y,head.y);
         })(A2($List.drop,1,snake.body));
      }();
   };
   var checkForSnakeAndFruitCollision = F2(function (snake,
   fruit) {
      return function () {
         var head = getSnakeHead(snake);
         return _U.eq(head.x,
         fruit.x) && _U.eq(head.y,
         fruit.y);
      }();
   });
   var Coord = function (a) {
      return {ctor: "Coord",_0: a};
   };
   ...
   var input = $Signal.mergeMany(_L.fromArray([A2($Signal.map,
                                              Space,
                                              $Keyboard.space)
                                              ,$Signal.sampleOn(delta)($Signal.map(Coord)(A2($Signal.map,
                                              function (a) {
                                                 return {_: {}
                                                        ,x: $Basics.toFloat(a.x)
                                                        ,y: $Basics.toFloat(a.y)};
                                              },
                                              $Keyboard.arrows)))]));
   ...
   var startLength = 3;
   var cellSize = 20;
   var borderSize = cellSize * 1;
   var defaultSnake = A2(Snake,
   RIGHT,
   _L.fromArray([A2(Cell,0,0)
                ,A2(Cell,-1 * cellSize,0)
                ,A2(Cell,-2 * cellSize,0)]));
   var paintFruit = function (f) {
      return $Graphics$Collage.move({ctor: "_Tuple2"
                                    ,_0: f.x
                                    ,_1: f.y})($Graphics$Collage.filled(fruitColor)(A2($Graphics$Collage.rect,
      cellSize,
      cellSize)));
   };
   var paintSnakeElement = F2(function (index,
   el) {
      return function () {
         var alpha = A3($Basics.clamp,
         0.7,
         1,
         1 - $Basics.toFloat(index) * 5.0e-2);
         return $Graphics$Collage.move({ctor: "_Tuple2"
                                       ,_0: el.x
                                       ,_1: el.y})($Graphics$Collage.filled(A4($Color.hsla,
         178,
         0.3,
         0.52,
         alpha))(A2($Graphics$Collage.rect,
         cellSize,
         cellSize)));
      }();
   });
   ...
   halfWidth = $._0,
   halfHeight = $._1;
   var randomFruit = F2(function (snake,
   seed0) {
      return function () {
         var $ = A2($Random.generate,
         A2($Random.$int,
         borderSize - halfWidth,
         halfWidth - borderSize),
         seed0),
         newX = $._0,
         seed1 = $._1;
         var $ = A2($Random.generate,
         A2($Random.$int,
         borderSize - halfHeight,
         halfHeight - borderSize),
         seed1),
         newY = $._0,
         seed2 = $._1;
         var roundedY = $Basics.toFloat($Basics.round($Basics.toFloat(newY) / cellSize)) * cellSize;
         var roundedX = $Basics.toFloat($Basics.round($Basics.toFloat(newX) / cellSize)) * cellSize;
         var isCellAlreadyOccupied = A2($List.any,
         function (a) {
            return _U.eq(a.x,
            roundedX) && _U.eq(a.y,
            roundedY);
         ...
         snake.body);
         return isCellAlreadyOccupied ? A2(randomFruit,
         snake,
         seed2) : {ctor: "_Tuple2"
                  ,_0: A2(Cell,roundedX,roundedY)
                  ,_1: seed2};
      }();
   });
   var defaultGame = function () {
      var seed = $Random.initialSeed(74811);
      var $ = A2(randomFruit,
      defaultSnake,
      seed),
      fruit = $._0,
   ...
   var updateSnake = F2(function (coord,
   snake) {
      return function () {
         var snakeHead = getSnakeHead(snake);
         var newDirection = _U.cmp(coord.y,
         0) > 0 && $Basics.not(_U.eq(snake.dir,
         DOWN)) ? UP : _U.cmp(coord.y,
         0) < 0 && $Basics.not(_U.eq(snake.dir,
         UP)) ? DOWN : _U.cmp(coord.x,
         0) < 0 && $Basics.not(_U.eq(snake.dir,
         RIGHT)) ? LEFT : _U.cmp(coord.x,
         0) > 0 && $Basics.not(_U.eq(snake.dir,
         LEFT)) ? RIGHT : snake.dir;
         var newX = _U.eq(newDirection,
         LEFT) ? snakeHead.x - cellSize : _U.eq(newDirection,
         RIGHT) ? snakeHead.x + cellSize : snakeHead.x;
         var newY = _U.eq(newDirection,
         DOWN) ? snakeHead.y - cellSize : _U.eq(newDirection,
         UP) ? snakeHead.y + cellSize : snakeHead.y;
         var newSnakeHead = {_: {}
                            ,x: A3($Basics.clamp,
                            borderSize - halfWidth,
                            halfWidth - borderSize,
                            ...
                            halfHeight - borderSize,
                            newY)};
         return $Debug.watch("Snake")(_U.replace([["dir"
                                                  ,newDirection]
                                                 ,["body"
                                                  ,A2($List._op["::"],
                                                  newSnakeHead,
                                                  $List.reverse($List.drop(1)($List.reverse(snake.body))))]],
         snake));
      }();
   });
               ...
               switch (_v4._0.ctor)
                 {case "Coord":
                    switch (_v4._1.ctor)
                      {case "Play":
                         return function () {
                              var isFruitCollision = A2(checkForSnakeAndFruitCollision,
                              _v2.snake,
                              _v2.fruit);
                              var newSnake0 = isFruitCollision ? incrementSnake(_v2.snake) : _v2.snake;
                              var newSnake1 = A2(updateSnake,
                              _v4._0._0,
                              newSnake0);
                              var $ = isFruitCollision ? A2(randomFruit,
                              _v2.snake,
                              _v2.seed) : {ctor: "_Tuple2"
                                          ,_0: _v2.fruit
                                          ,_1: _v2.seed},
                              newFruit = $._0,
                              newSeed = $._1;
                              var isSnakeCollision = checkForSnakeCollision(_v2.snake);
                              return _U.replace([["state"
                                                 ,$Basics.not(isSnakeCollision) ? Play : Lose]
                                                ,["snake"
                                                 ,isSnakeCollision ? _v2.snake : newSnake1]
                                                ,["fruit",newFruit]
                                                ,["seed",newSeed]],
                              _v2);
                           }();}
                    ...
                    switch (_v4._1.ctor)
                      {case "Lose":
                         return _v4._0._0 ? defaultGame : _v2;
                         case "NewGame":
                         return _U.replace([["state"
                                            ,_v4._0._0 ? Play : _v2.state]],
                           _v2);}
                      break;}
                 break;}
            return _v2;
   ...
   _v10) {
      return function () {
         return function () {
            ...
            {case "_Tuple2":
               return function () {
                    var score = ($List.length(_v10.snake.body) - startLength) * 10;
                    var lable = _U.eq(_v10.state,
                    Lose) ? A2($Basics._op["++"],
                    "Your score is ",
                    A2($Basics._op["++"],
                    $Basics.toString(score),
                    ". Press SPACE.")) : _U.eq(_v10.state,
                    NewGame) ? "Press SPACE to start, use ARROWS to controll" : "";
                    var textLabel = $Graphics$Collage.move({ctor: "_Tuple2"
                                                           ,_0: 0
                                                           ,_1: borderSize * 3 - halfHeight})($Graphics$Collage.toForm(A2(txt,
                    $Text.height(20),
                    lable)));
                    ...
                    gameWidth,
                    gameHeight,
                    $List.append(_L.fromArray([$Graphics$Collage.filled(fieldColor)(A2($Graphics$Collage.rect,
                                              gameWidth,
                                              gameHeight))
                                              ...
                                              gameWidth,
                                              gameHeight))
                                              ,paintFruit(_v10.fruit)]))($List.append(paintSnake(_v10.snake))(_L.fromArray([textLabel])))));
                 }();}
            _U.badCase($moduleName,
            "between lines 171 and 199");
         }();
      }();
   });
   ...
   _elm.SnakeGame.values = {_op: _op
                           ...
                           ,update: update
                           ,checkForSnakeCollision: checkForSnakeCollision
                           ,checkForSnakeAndFruitCollision: checkForSnakeAndFruitCollision
                           ,incrementSnake: incrementSnake
                           ,updateSnake: updateSnake
                           ,view: view
                           ,paintFruit: paintFruit
                           ,paintSnake: paintSnake
                           ,paintSnakeElement: paintSnakeElement
                           ,txt: txt
